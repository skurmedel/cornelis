...
auto distributionGTR2(float cos_theta_H, float alpha) -> float {
    ...
    if (isAlmostZero(alpha2))
        return 1.0f;
    // These are arbitrarly named terms. They serve just to partition the expression.
    float A = alpha2 / (2.0f * Pi);
    float B = 1.0f / std::pow(1.0f + (alpha2 - 1.0f) * cos_theta_H2, 2.0f);
...
auto lambdaTR(float tan_theta, float alpha) -> float {
    if (std::isinf(tan_theta))
        return 0.0f;
    return (-1.0f + sqrtf(1.0f + (abs(tan_theta) * alpha) * (abs(tan_theta) * alpha))) * 0.5f;
}
